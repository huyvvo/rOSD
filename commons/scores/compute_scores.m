...
for cl = class_indices
  ...
  for row = row_indices
    ...
    [confidence, standout] = compute_scores_(opts, i, j);
    if opts.saveconf
...
function [feati, featj] = read_feat(opts, i, j)
  ...
  if opts.deepfeat
    ...
    featj = process_deepfeat(featj, opts.sim_type);
  end
...
function [confidence, standout] = compute_scores_(opts, i, j)
  [feati, featj] = read_feat(opts, i, j);
  % compute standout
  timer_conf = tic;
  if opts.symmetric
