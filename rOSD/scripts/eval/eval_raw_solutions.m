...
% EVAL_RAW_SOLUTIONS
% [sol, corloc_nu, corloc_5, corloc_1] = eval_raw_solutions(args, class_indices)
%
...
for cl = class_indices
  ...
  sol = struct('x_opt_nu', {[]}, 'x_opt_5', {[]}, 'x_opt_1', {[]});
  opts.sol_buffer = get_first_solution_batch(opts);
  opts.sol_buffer = get_solutions(opts, opts.sol_buffer, 1, opts.num_iters);
  %----------------------------------------------------
  opts.do_postprocessing = ~isfield(opts.sol_buffer, 'x_opt_5') || ...
                           ~isfield(opts.sol_buffer, 'x_opt_1') || ...
                           ~isfield(opts.sol_buffer, 'nms_IoU') || ...
                           opts.sol_buffer.nms_IoU ~= nms_IoU;
  if opts.do_postprocessing
    fprintf('Creating data loaders and loading scores ...\n');
    opts.DL = opts.data_loader(load(opts.score_path));
  end
  %----------------------------------------------------
  for iter = 1:opts.num_iters
    [x_opt_nu, x_opt_5, x_opt_1] = process_solutions(opts, iter);
  ...
  if numel(sol_buffer.x_s) < r | isempty(sol_buffer.x_s{l}) | isempty(sol_buffer.x_s{r})
    for i = 1:numel(files)
      ...
      if numel(intersect([l:r], [ids(1):ids(2)])) > 0
        ...
        if isfield(sol_buffer, 'x_opt_5')
          sol_buffer.x_opt_5(ids(1):ids(2)) = current_sol.x_opt_5(ids(1):ids(2));
        end
        if isfield(sol_buffer, 'x_opt_1')
          sol_buffer.x_opt_1(ids(1):ids(2)) = current_sol.x_opt_1(ids(1):ids(2));
        end
      end
    end
  end
...
function [x_opt_nu, x_opt_5, x_opt_1, e_opt_1] = process_solutions(opts, iter)
  ...
  if opts.do_postprocessing
    x_opt_5 = post_proc_nms(opts.DL, opts.sol_buffer.x_s{iter}, opts.sol_buffer.e_s{iter}, ...
                            5, opts.proposals, tau, 1, opts.group_ids, nms_IoU);
    x_opt_1 = post_proc_nms(opts.DL, opts.sol_buffer.x_s{iter}, opts.sol_buffer.e_s{iter}, ...
                            1, opts.proposals, tau, 1, opts.group_ids, nms_IoU);
  else 
    x_opt_5 = opts.sol_buffer.x_opt_5{iter};
    x_opt_1 = opts.sol_buffer.x_opt_1{iter};
  end
end 
...
function [] = save_result(opts, corloc, detRate, sol)
  ...
  save(fullfile(opts.save_path, ...
                sprintf('corloc_nms_IoU_0%s_raw_%d_solutions.mat', num2str(10*opts.nms_IoU), opts.num_iters)), ...
       '-struct', 'corloc');
  save(fullfile(opts.save_path, ...
                sprintf('detection_rate_nms_IoU_0%s_raw_%d_solutions.mat', num2str(10*opts.nms_IoU), opts.num_iters)), ...
       '-struct', 'detRate');
  fprintf('DONE!\n');
end
