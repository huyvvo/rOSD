...
for cl = class_indices
  ...
  if opts.vis_result
    imdb = load(fullfile(opts.images_root, opts.clname, [opts.clname, '.mat']), 'images');
    imdb.proposals = opts.proposals;
    imdb.bboxes = opts.bboxes;
    visualize_result(imdb, fullfile(opts.vis_root, opts.clname), ...
...
function [args] = argument_reader(args)
  assert(all(isfield(args, {...
                'root', 'clname', 'small_imdb', 'lite_imdb', 'nu', 'tau', 'nms_IoU', 'num_ascent', ...
...
function [x_s, e_s, x_opt_5, x_opt_1, e_opt_1] = get_solutions(opts)
  n = opts.n; nu = opts.nu; tau = opts.tau; 
  if isfield(opts, 'DL') == 1
    [x_s, e_s] = round_x_e_fast(opts.DL, opts.x, opts.e, opts.e_rounding, nu, tau, ...
                           opts.group_ids, randperm(n,n));
    if opts.num_ascent > 0
      [x_s, e_s] = ascent_x_e_fast(opts.DL, x_s, e_s, opts.e_iterating, nu, tau, ...
                              opts.num_regions, opts.group_ids, opts.num_ascent);
    end
    ...
    [x_s, e_s] = round_x_e_fast(opts.rounding_DL, opts.x, opts.e, opts.e_rounding, nu, tau, ...
                           opts.group_ids, randperm(n,n));
    if opts.num_ascent > 0
      [x_s, e_s] = ascent_x_e_fast(opts.iterating_DL, x_s, e_s, opts.e_iterating, nu, tau, ...
                              opts.num_regions, opts.group_ids, opts.num_ascent);
    end
