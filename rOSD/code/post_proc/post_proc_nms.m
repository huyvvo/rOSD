...
for i = 1:n
  ...
  x_opt{i} = nms_x(x{i}(region_ids), proposals{i}, group_ids{i}, k, nms_IoU);
  if k == 1
...
%-------------------------------------------
function [x_s] = nms_x(top_ids, prop_list, group_ids_list, nu, nms_IoU)
  ...
  for iter = 1:numel(top_ids) 
    ...
    if ~isempty(x_s) & max(bbox_iou(prop_list(top_ids(iter),:), prop_list(x_s,:))) > nms_IoU
      continue;
    end
