...
for cl = class_indices
  ...
  corloc_5_class = [];
  corloc_1_class = [];
  sol = struct('x_s', {[]}, 'e_s', {[]}, 'x_opt_5', {[]}, 'x_opt_1', {[]}, 'nms_IoU', opts.nms_IoU);
  for i = solution_indices
    if mod(i, 100) == 1
      fprintf('Computing solution %d\n', i);
    end
    tic;
    [sol.x_s{i}, sol.e_s{i}, sol.x_opt_5{i}, sol.x_opt_1{i}] = get_solutions(opts);
    corloc_nu_class(end+1) = CorLoc(opts.proposals, opts.bboxes, sol.x_s{i}, 0.5);
    corloc_5_class(end+1) = CorLoc(opts.proposals, opts.bboxes, sol.x_opt_5{i}, 0.5);
    corloc_1_class(end+1) = CorLoc(opts.proposals, opts.bboxes, sol.x_opt_1{i}, 0.5);
  ...
  if opts.vis_result
    visualize_result(imdb_path, fullfile(opts.vis_root, opts.clname), x_s, x_s, x_opt_1);
  end
  %------------------------------
  % save results
...
function [x_s, e_s, x_opt_5, x_opt_1] = get_solutions(opts)
  ...
  num_regions = opts.num_regions; nms_IoU = opts.nms_IoU;
  if isfield(opts, 'DL') == 1
    [x_s, e_s] = round_x_e(opts.DL, opts.x, opts.e, opts.e_rounding, nu, tau, randperm(n,n));
    [x_s, e_s] = ascent_x_e(opts.DL, x_s, e_s, opts.e_iterating, nu, tau, num_regions, 10);
    x_opt_5 = post_proc_nms(opts.DL, x_s, e_s, 5, opts.proposals, tau, 1, nms_IoU);
    x_opt_1 = post_proc_nms(opts.DL, x_s, e_s, 1, opts.proposals, tau, 1, nms_IoU);
  else 
    [x_s, e_s] = round_x_e(opts.rounding_DL, opts.x, opts.e, opts.e_rounding, nu, tau, randperm(n,n));
    [x_s, e_s] = ascent_x_e(opts.iterating_DL, x_s, e_s, opts.e_iterating, nu, tau, num_regions, 10);
    x_opt_5 = post_proc_nms(opts.iterating_DL, x_s, e_s, 5, opts.proposals, tau, 1, nms_IoU);
    x_opt_1 = post_proc_nms(opts.iterating_DL, x_s, e_s, 1, opts.proposals, tau, 1, nms_IoU);
  end
end 
